  ...
  // sanitize USER INPUT ONLY (name, phone, etc.) — never call this on the full message template
  function _sanitize(str) {
    ...
    return str
      ...
      .replace(/"/g, '&quot;')
      .replace(/'/g, '&#x27;')
      .slice(0, 300); // cap user input fields
  }
  ...
  async function sendTelegramMessage(text, replyMarkup = null) {
    ...
    // Pass the full message as-is (it already contains trusted HTML from our templates)
    const safeText = (typeof text === 'string') ? text.slice(0, 4096) : '';
  ...
  function submitBooking() {
    ...
    pendingBooking = {
      name: _sanitize(name), phone: '+998 ' + _sanitize(phoneRaw), pcType, durText, datetime, total: totalFormatted,
      people: peopleCount, extraGuests: extraGuests.map(g => ({ name: _sanitize(g.name), phone: _sanitize(g.phone) }))
    };
  ...
  async function submitCreateAccount() {
    ...
    const safePhone = _sanitize(phone);
    const phoneDigits = safePhone.replace(/[\s\-]/g, '');

    const uzbekMsg = `Salom ${safeName}! 👋\n\nEnergy Gaming UZ'da akkauntingiz muvaffaqiyatli yaratildi! ✅\n\nBiz bilan o'ynashdan bahramand bo'ling! 🎮`;

    const msg =
      `🆕 <b>YANGI AKKAUNT SO'ROVI — Energy Gaming UZ</b>\n\n` +
      `👤 Ism: ${safeName}\n` +
      `📞 Telefon: <a href="https://t.me/+998${phoneDigits}">+998 ${safePhone}</a>\n` +
      `🕐 So'rov vaqti: ${now}\n\n` +
